...
export default function ForceFreshButton() {
  ...
  return (
    <div>
      ...
      >
        {busy ? 'Revoking…' : '⚡ Revoke & restart Instagram Login'}
      </button>
      {error && (
      ...
      <p className="text-[10px] text-amber-900/70 mt-2 leading-snug">
        Revokes the app&apos;s Instagram permissions, then reopens the Business
        Login for Instagram flow with <code>force_reauth=true</code> so the full
        login + permission-grant screen is shown again.
      </p>
    </div>
  );
}
