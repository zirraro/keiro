...
export async function POST(req: NextRequest) {
  ...
  const urls = imgs.slice(0, 5).map(i => i.largeImageURL).filter(Boolean);
  if (urls.length < 3) return NextResponse.json({ error: 'not_enough_pixabay_images', got: urls.length }, { status: 502 });
