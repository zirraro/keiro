...
${String(texteDuPost).slice(0, 500)}` : '',
    ...
    // générateur : le journal les note depuis des mois, personne ne les
    // relisait. Agrégation en cache, aucun appel de modèle.
    let apprentissage = '';
    try {
      const { consignesTireesDesRefus } = await import('@/lib/visuals/apprentissage-qc');
      apprentissage = await consignesTireesDesRefus(getSupabaseAdmin(), {
        userId: userIdForReuse, plateforme: platformForReuse,
      });
  ...
  try {
    ...
    try {
      ...
      if (ttUid) {
        ...
        // DÉFAUT = AUTO (nos posts auto SONT utiles — c'est le produit). Le mode
        // 'manual' n'est qu'un CHOIX EXPLICITE du client (booster son tendance),
        // jamais un défaut imposé, jamais "l'auto ne sert à rien".
...
- LA MÉTAPHORE EST PERMISE, MAIS ELLE DOIT ÊTRE ANCRÉE. Une image métaphorique est un jeu de mots visuel : c'est un bon ressort quand il est juste. Ce qui ne marche pas, c'est la métaphore INTERCHANGEABLE — mesuré le 19 août, la moitié des notes basses venait de là : un papier froissé pour « l'occasion ratée », un carré de papier pour « la page blanche ». Ces images conviendraient à n'importe quel commerce, donc elles ne parlent à personne.
  Le test : la métaphore emprunte-t-elle son objet AU MÉTIER, à sa cible ou à l'actualité utilisée ? Une pâte qui retombe pour un boulanger qui parle d'élan perdu, une mèche coupée net pour un coiffeur qui parle de décision — oui, l'objet vient du métier. Un papier froissé pour un fleuriste — non, il vient de nulle part.
  Autrement dit : garde le jeu de mots, prends-le dans le monde du client.
