...
Reponds en francais, sois direct et actionnable.`;
    ...
    // 2026-06-08 — Load Noah's typed directives for the calling user
    let noahDirectives = '';
    try {
      const { directiveBlockFor } = await import('@/lib/agents/typed-directives');
      const noahUserId = (founderProfile as any)?.id || null;
      noahDirectives = await directiveBlockFor(supabase, noahUserId, 'ceo');
    } catch { /* best-effort */ }
...
export const GET = avecContexteRoute('ceo', GETInterne);
export const POST = avecContexteRoute('ceo', POSTInterne);
