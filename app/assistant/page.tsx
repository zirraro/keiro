        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-3">
              <h1 className="text-white font-bold text-2xl lg:text-3xl">
                {en ? 'Workspace' : 'Espace de travail'}
              </h1>
              {isAdmin && (
                <>
              ...
              {!isVisitor && (
                <>
                  ...
                  >
                    ...
                    {en ? 'My business file' : 'Mon dossier'}
                  </button>
                  <button
                    onClick={() => router.push('/assistant/crm')}
                    className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-white/60 text-xs transition-colors"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {summary?.globalStats && !COMING_SOON_MODE && !isVisitor ? (
            <div className="flex items-center gap-3 mt-1 flex-wrap">
              <span className="text-white/50 text-sm">{activeAgents} {en ? 'active agents' : 'agents actifs'}</span>
              <span className="text-white/45">|</span>
              <span className="text-cyan-400 text-sm font-medium">+{summary.globalStats.prospectsToday || 0} {en ? 'prospects' : 'prospects'}</span>
              <span className="text-white/45">|</span>
              <span className="text-blue-400 text-sm font-medium">{summary.globalStats.emailsSent || 0} {en ? 'emails' : 'emails'}</span>
              <span className="text-white/45">|</span>
              <span className="text-purple-400 text-sm font-medium">{summary.globalStats.contentPublished || 0} {en ? 'posts' : 'posts'}</span>
              {summary.globalStats.emailOpenRate > 0 && (
                <>
                  <span className="text-white/45">|</span>
                  <span className="text-emerald-400 text-sm font-medium">{summary.globalStats.emailOpenRate}% {en ? 'open rate' : 'ouverture'}</span>
                </>
              )}
              {summaryUpdatedAt && (
                <>
                  ...
                  <span className="text-white/30 text-[11px] flex items-center gap-1" title={summaryUpdatedAt.toLocaleString(en ? 'en-GB' : 'fr-FR')}>
                    ...
                    {en ? 'Updated' : 'Maj'} {summaryUpdatedAt.toLocaleTimeString(en ? 'en-GB' : 'fr-FR', { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </>
              )}
            </div>
            ...
            <p className="text-white/50 text-sm">
              {COMING_SOON_MODE ? `${agents.length} ${en ? 'AI agents' : 'agents IA'}` : `${activeAgents} ${en ? 'active agents' : 'agents actifs'}`}
            </p>
          )}
