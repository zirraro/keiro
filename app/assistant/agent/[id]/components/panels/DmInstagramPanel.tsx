...
function ManualFollowsList({ platform = 'instagram' }: { platform?: 'instagram' | 'tiktok' | 'linkedin' }) {
  ...
  const profileHref = (handle: string) => {
    ...
    if (platform === 'linkedin') {
      // Anti-lien-mort (founder 25/07) : on n'ouvre en DIRECT que les vraies URL
      // de profil/entreprise LinkedIn (/in/ ou /company/). Toute autre valeur
      // (slug deviné au scraping, souvent mort) → recherche LinkedIn, qui atterrit
      // toujours sur des résultats réels au lieu d'un 404.
      const raw = String(handle || '').trim();
      if (/linkedin\.com\/(in|company)\/[^/?#\s]+/i.test(raw)) return /^https?:\/\//i.test(raw) ? raw : `https://www.${raw.replace(/^\/+/, '')}`;
      return `https://www.linkedin.com/search/results/all/?keywords=${h}`;
    }
