    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 min-h-[400px] lg:h-[calc(100vh-280px)]">
      ...
      <div className="lg:col-span-2 flex flex-col bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-2 px-3 py-2 border-b border-white/10 bg-white/[0.02]">
          ...
          />
          {originalFormat !== 'csv' && (
            <span className="text-xs text-white/30 uppercase">{originalFormat}</span>
          )}
          {hasFormulasRef.current && (
            <span className="text-xs text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded" title="Formules preservees">fx</span>
          )}
          {hasMacrosRef.current && (
            <span className="text-xs text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded" title="Macros VBA preservees">VBA</span>
          )}
        ...
        <div className="flex-1 overflow-auto p-3">
          <table className="border-collapse w-full">
            <tbody>
              {grid.map((row, rowIdx) => (
                <tr key={rowIdx}>
                  <td className="text-xs text-white/30 px-1 py-0.5 text-center select-none w-8">{rowIdx + 1}</td>
                  {row.map((cell, colIdx) => (
      ...
      <div className="flex flex-col bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden">
        <div className="px-3 py-2 border-b border-white/10 bg-white/[0.02]">
          ...
          <p className="text-xs text-white/40">Il calcule et modifie en live</p>
        </div>
            ...
            <div className="text-center py-8 text-white/30 text-xs">
              ...
              <div className="space-y-1.5">
                {[
                  ...
                  <button key={ex} onClick={() => setChatInput(ex)} className="block w-full text-xs text-white/40 hover:text-white/70 hover:bg-white/5 px-2 py-1.5 rounded transition text-left">
                    {`\u201C${ex}\u201D`}
                  </button>
                ))}
              </div>
