    ...
    return (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 text-center">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-cyan-500 to-[#1e3a5f] rounded-full mb-4">
          ...
          <div className="space-y-2.5 mb-6">
            <button
              ...
              className="w-full px-5 py-3 bg-gradient-to-r from-cyan-500 to-[#1e3a5f] text-white font-semibold rounded-lg hover:shadow-lg transition-all text-sm"
            >
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b bg-gradient-to-r from-cyan-50 to-[#0c1a3a]/5">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="w-8 h-8 bg-gradient-to-r from-cyan-500 to-[#1e3a5f] rounded-lg flex items-center justify-center">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 px-4 py-2 rounded-lg font-semibold text-sm transition-all ${
                activeTab === 'videos'
                  ? 'bg-gradient-to-r from-cyan-500 to-[#1e3a5f] text-white shadow-md'
        ...
        <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
              ...
              {activeTab === 'videos' ? (
                ...
                availableVideos.map((vid) => (
                  <button key={vid.id} onClick={() => setSelectedVideo(vid)}
                    className={`w-14 h-14 flex-shrink-0 rounded-lg overflow-hidden transition-all relative ${selectedVideo?.id === vid.id ? 'ring-2 ring-cyan-500 scale-105' : 'ring-1 ring-neutral-300'}`}>
                ...
                availableImages.map((img) => (
                  <button key={img.id} onClick={() => setSelectedImage(img)}
                    className={`w-14 h-14 flex-shrink-0 rounded-lg overflow-hidden transition-all ${selectedImage?.id === img.id ? 'ring-2 ring-cyan-500 scale-105' : 'ring-1 ring-neutral-300'}`}>
                    ...
                    availableVideos.map((vid) => (
                      ...
                      >
                        {vid.thumbnail_url ? (
                          ...
                          <div className="w-full h-full bg-gradient-to-br from-cyan-50 to-[#0c1a3a]/5 flex flex-col items-center justify-center gap-2">
                    ...
                    availableVideos.map((vid) => (
                      ...
                      >
                        {vid.thumbnail_url ? (
                          ...
                          <div className="w-full h-full bg-gradient-to-br from-cyan-50 to-[#0c1a3a]/5 flex flex-col items-center justify-center gap-1">
          ...
          <div className="flex-1 p-4 sm:p-6 overflow-y-auto bg-gradient-to-br from-cyan-50 to-[#0c1a3a]/5">
              ...
              <div className="max-w-[280px] mx-auto">
                ...
                {activeTab === 'images' && selectedImage && (
                  <button
                    ...
                    className="w-full mb-2 py-2 bg-[#0c1a3a]/5 text-[#0c1a3a] text-xs font-semibold rounded-lg hover:bg-[#0c1a3a]/10 transition flex items-center justify-center gap-1.5"
                  >
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {hashtags.map(tag => (
                    <span
                      key={tag}
                      className="inline-flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-cyan-100 to-[#0c1a3a]/10 text-cyan-800 text-xs rounded-full"
                    >
                      {tag}
                      <button
            ...
            {isTikTokConnected ? (
                ...
                <button
                  ...
                  className={`flex-1 px-3 sm:px-4 py-2 sm:py-3 rounded-lg font-medium text-white transition-all flex items-center justify-center gap-1 sm:gap-2 text-xs sm:text-sm ${
                    publishing || !caption.trim() || (!isVideo(selectedImage?.image_url || '') && !videoPreview)
                      ? 'bg-neutral-400 cursor-not-allowed'
                      : 'bg-gradient-to-r from-cyan-500 to-[#1e3a5f] hover:from-cyan-600 hover:to-[#0c1a3a] shadow-lg hover:shadow-xl'
                  }`}
              ...
              <button
                ...
                className={`flex-1 px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-medium text-white transition-colors flex items-center justify-center gap-2 text-sm sm:text-base ${
                  saving || !caption.trim()
                    ? 'bg-neutral-400 cursor-not-allowed'
                    : 'bg-gradient-to-r from-cyan-500 to-[#1e3a5f] hover:from-cyan-600 hover:to-[#0c1a3a]'
                }`}
              >
                {saving ? (
                  <>
            ...
            {loadingCreatorInfo ? (
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="bg-gradient-to-r from-pink-50 to-cyan-50 rounded-xl p-4 border border-pink-200/50">
                  <div className="flex items-center gap-3">
                    ...
                    {creatorInfo?.can_post ? (
                      <span className="ml-auto text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full font-medium">{en ? 'Ready to publish' : 'Prêt à publier'}</span>
                    ) : (
                      <span className="ml-auto text-xs bg-red-100 text-red-700 px-2 py-1 rounded-full font-medium">{en ? 'Publishing unavailable' : 'Publication indisponible'}</span>
                    )}
                  </div>
                  {!creatorInfo?.can_post && (
                    <p className="text-xs text-red-600 mt-2">{en ? 'TikTok indicates publishing is unavailable for this account. Please try again later.' : 'TikTok indique que la publication n\u2019est pas disponible pour ce compte actuellement. Veuillez réessayer plus tard.'}</p>
                  )}
                </div>

                {/* Content Preview — TikTok phone frame */}
                <div>
                  <label className="block text-xs font-semibold text-neutral-700 mb-2">{en ? 'Post preview' : 'Aperçu de la publication'}</label>
                ...
                <div>
                  ...
                  {!privacyLevel && (
                    <p className="text-xs text-amber-600 mb-2">{en ? 'Please select a privacy level' : 'Veuillez sélectionner un niveau de confidentialité'}</p>
                  )}
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            className="w-4 h-4 text-[#0c1a3a]"
                          />
                          <div>
                            <p className="text-sm font-medium text-neutral-900">
                              {option === 'PUBLIC_TO_EVERYONE' ? (en ? 'Public' : 'Public') : option === 'MUTUAL_FOLLOW_FRIENDS' ? (en ? 'Friends' : 'Amis') : option === 'FOLLOWER_OF_CREATOR' ? (en ? 'Followers' : 'Abonnés') : option === 'SELF_ONLY' ? (en ? 'Only me' : 'Moi uniquement') : option}
                            </p>
                            <p className="text-xs text-neutral-500">
                              {option === 'PUBLIC_TO_EVERYONE' ? (en ? 'Everyone can view' : 'Tout le monde peut voir') : option === 'MUTUAL_FOLLOW_FRIENDS' ? (en ? 'Only your mutual friends' : 'Seuls vos amis mutuels') : option === 'FOLLOWER_OF_CREATOR' ? (en ? 'Your followers only' : 'Vos abonnés uniquement') : option === 'SELF_ONLY' ? (en ? 'Only visible to you' : 'Visible uniquement par vous') : ''}
                            </p>
                            {disabledByBrandContent && (
                              <p className="text-xs text-amber-600 mt-0.5">{en ? 'Branded content visibility cannot be set to private' : 'Le contenu de marque ne peut pas être défini en privé'}</p>
                            )}
                          </div>
                        </label>
                      );
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-lg border border-neutral-200">
                    <button
                      ...
                      className={`relative w-11 h-6 rounded-full transition-colors ${contentDisclosure ? 'bg-[#0c1a3a]' : 'bg-neutral-300'}`}
                    >
                    ...
                    <div>
                      <p className="text-sm font-medium text-neutral-700">{en ? 'This content promotes goods or services' : 'Ce contenu promeut un bien ou un service'}</p>
                      <p className="text-xs text-neutral-500">{en ? 'Turn on to disclose that this content promotes goods or services in exchange for something of value' : 'Active si ce contenu promeut un produit ou service en échange d\u2019une contrepartie'}</p>
                    </div>
                  </div>
                  ...
                  {contentDisclosure && (
                    <div className="mt-3 space-y-2">
                      {/* Your Brand */}
                      <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition ${brandOrganic ? 'border-[#0c1a3a] bg-[#0c1a3a]/5' : 'border-neutral-200 hover:bg-neutral-50'}`}>
                        <input type="checkbox" checked={brandOrganic} onChange={(e) => setBrandOrganic(e.target.checked)} className="w-4 h-4 rounded text-[#0c1a3a]" />
                        <div>
                          <p className="text-sm font-medium text-neutral-700">{en ? 'Your Brand' : 'Votre marque'}</p>
                          <p className="text-xs text-neutral-500">{en ? 'You are promoting yourself or your own business' : 'Vous faites la promotion de vous-même ou de votre propre entreprise'}</p>
                          {brandOrganic && <p className="text-xs text-[#0c1a3a] mt-1">{en ? 'Your photo/video will be labeled as "Promotional content"' : 'Votre photo/vidéo sera étiquetée « Contenu promotionnel »'}</p>}
                        </div>
                      </label>
                      {/* Branded Content */}
                      <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition ${
                        (privacyLevel === 'SELF_ONLY' || privacyLevel === 'FOLLOWER_OF_CREATOR')
                          ...
                          : brandContent ? 'border-[#0c1a3a] bg-[#0c1a3a]/5' : 'border-neutral-200 hover:bg-neutral-50'
                      }`}>
                        <input
                          type="checkbox"
                          ...
                          className="w-4 h-4 rounded text-[#0c1a3a]"
                        />
                        <div>
                          <p className="text-sm font-medium text-neutral-700">{en ? 'Branded Content' : 'Contenu de marque'}</p>
                          <p className="text-xs text-neutral-500">{en ? 'You are promoting another brand or a third party' : 'Vous faites la promotion d\u2019une autre marque ou d\u2019un tiers'}</p>
                          {brandContent && <p className="text-xs text-[#0c1a3a] mt-1">{en ? 'Your photo/video will be labeled as "Paid partnership"' : 'Votre photo/vidéo sera étiquetée « Partenariat rémunéré »'}</p>}
                          {(privacyLevel === 'SELF_ONLY' || privacyLevel === 'FOLLOWER_OF_CREATOR') && (
                            <p className="text-xs text-amber-600 mt-1">{en ? 'Branded content cannot be set to private. Select Public or Friends visibility above.' : 'Le contenu de marque ne peut pas être défini en privé. Choisis Public ou Amis ci-dessus.'}</p>
                          )}
                        </div>
                      </label>

                      {contentDisclosure && !brandOrganic && !brandContent && (
                        <p className="text-xs text-red-500 mt-1">{en ? 'Please select at least one option when content disclosure is enabled' : 'Sélectionne au moins une option quand la divulgation est activée'}</p>
                      )}
                    </div>
                  )}
                </div>
                ...
                <div className="border-t pt-4 space-y-3">
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    <input type="checkbox" checked={legalAccepted} onChange={(e) => setLegalAccepted(e.target.checked)} className="w-4 h-4 rounded text-[#0c1a3a] mt-0.5" />
                    <p className="text-xs text-neutral-600">
                      I agree to TikTok&apos;s{' '}
                      <a href="https://www.tiktok.com/legal/page/global/music-usage-confirmation/en" target="_blank" rel="noopener noreferrer" className="text-[#0c1a3a] underline">
                        Music Usage Confirmation
                      </a>
                      {(brandContent) && (
                        <>{' '}and the{' '}
                          <a href="https://www.tiktok.com/legal/page/global/bc-policy/en" target="_blank" rel="noopener noreferrer" className="text-[#0c1a3a] underline">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-cyan-500 to-[#0c1a3a] rounded-xl hover:shadow-lg transition disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    {publishing ? (
                      <span className="flex items-center justify-center gap-2">
