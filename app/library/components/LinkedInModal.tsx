...
export default function LinkedInModal({ image, images, video, videos, onClose, onSave, draftCaption, draftHashtags, linkedinConnected, onPublish }: LinkedInModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[70] flex items-end sm:items-center justify-center sm:p-4">
      <div className="bg-white w-full max-h-[92dvh] sm:w-[95vw] sm:max-w-6xl md:h-[88vh] rounded-t-2xl sm:rounded-2xl overflow-hidden flex flex-col shadow-2xl">
        {/* Header */}
                    ...
                    {activeTab !== 'text-only' && (
                      <div className="aspect-video bg-black">
                        {activeTab === 'videos' && selectedVideo ? (
                          ...
                          <DisplayImage src={selectedImage.image_url} alt="Preview" width={1200} className="w-full h-full object-cover" />
                        ) : null}
                      </div>
                    )}
