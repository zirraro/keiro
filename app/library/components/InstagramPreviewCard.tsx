...
export default function InstagramPreviewCard({ user, isGuest, draftCount, onOpenModal, onStartFree }: InstagramPreviewCardProps) {
  ...
  // Visitor version (neither user nor guest) - Feature showcase
  if (!user && !isGuest) {
    return (
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                <div className="flex items-center gap-1.5 text-white/90">
                  ...
                  <span className="text-xs font-medium">{t.library.ipcSuggestions}</span>
                </div>
                <div className="flex items-center gap-1.5 text-white/90">
                  <svg className="w-4 h-4 text-green-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-xs font-medium">{t.library.ipcDrafts}</span>
                </div>
                <div className="flex items-center gap-1.5 text-white/90">
                  <svg className="w-4 h-4 text-green-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-xs font-medium">{t.library.ipcHashtags}</span>
                </div>
                <div className="flex items-center gap-1.5 text-white/90">
                  <svg className="w-4 h-4 text-green-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-xs font-medium">{t.library.ipcScheduleSoon}</span>
                </div>
              </div>
              ...
              >
                <span>{t.library.ipcStartFree}</span>
  ...
  // Logged-in user or guest - Compact card
  return (
    <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl border border-purple-200 p-5 mb-6 hover:shadow-lg transition-shadow">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-neutral-900 text-lg">{t.library.ipcStudioInsta}</h3>
            <p className="text-sm text-neutral-600">
              {draftCount > 0 ? t.library.ipcDraftsPending.replace('{n}', String(draftCount)) : t.library.ipcNoDrafts}
            </p>
          </div>
        </div>
        <button
        ...
        >
          ...
          <span className="hidden sm:inline">{t.library.ipcPreparePost}</span>
          <span className="sm:hidden">{t.library.ipcNew}</span>
        </button>
      </div>
    </div>
  );
