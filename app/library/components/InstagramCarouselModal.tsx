...
export default function InstagramCarouselModal({ images, onClose }: InstagramCarouselModalProps) {
  ...
  const handleSuggest = async () => {
    if (selectedImages.length === 0) {
      alert(t.library.icmAlertSelectImage);
      return;
    }
  ...
  const handlePublishCarousel = async () => {
    ...
    if (!caption.trim()) {
      alert(t.library.icmAlertWriteDesc);
      return;
    }

    if (!isInstagramConnected) {
      alert(t.library.icmAlertConnectFirst);
      return;
    }

    const confirm = window.confirm(
      `${t.library.icmConfirmPublish}\n\n` +
      `${selectedImages.length} ${t.library.icmConfirmSelected}\n` +
      `${t.library.icmConfirmImmediate}\n\n` +
      `${t.library.icmConfirmContinue}`
    );
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b bg-gradient-to-r from-purple-50 to-[#0c1a3a]/5">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="w-8 h-8 bg-gradient-to-r from-[#0c1a3a] to-[#1e3a5f] rounded-lg flex items-center justify-center">
              <span className="text-xl">📸</span>
            </div>
            <div>
              <h2 className="text-lg sm:text-2xl font-bold text-neutral-900">{t.library.icmTitle}</h2>
              <p className="text-xs text-neutral-600">{t.library.icmSelectRange}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="flex-1 p-4 sm:p-6 overflow-y-auto bg-neutral-50">
            <div className="mb-4 bg-gradient-to-r from-purple-100 to-[#0c1a3a]/10 border border-purple-200 rounded-lg p-3">
              <p className="text-sm font-semibold text-purple-900 mb-1">
                📸 {selectedImages.length}/10 {t.library.icmSelectedCount}
              </p>
              <p className="text-xs text-purple-800">
                {t.library.icmClickToSelect}
              </p>
            </div>
            ...
            <div className="p-4 sm:p-6 space-y-6">
              ...
              {selectedImages.length > 0 && (
                <div>
                  <label className="block text-sm font-semibold text-neutral-900 mb-2">
                    {t.library.icmSelectedImages} ({selectedImages.length})
                  </label>
              ...
              <div>
                ...
                >
                  ...
                  <option value="emotionnel">❤️ {t.library.angleEmotional}</option>
              ...
              <div>
                <label className="block text-sm font-semibold text-neutral-900 mb-2">
                  {t.library.icmCarouselDescription}
                </label>
                <textarea
                  value={caption}
                  onChange={(e) => setCaption(e.target.value)}
                  rows={6}
                  placeholder={t.library.icmCarouselPlaceholder}
              ...
              <div>
                <label className="block text-sm font-semibold text-neutral-900 mb-2">
                  {t.library.icmSuggestedHashtags}
                </label>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {hashtags.map(tag => (
                    <span
                      key={tag}
                      className="inline-flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-purple-100 to-[#0c1a3a]/10 text-purple-800 text-xs rounded-full"
                    >
                      {tag}
                      <button
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder={t.library.icmAddHashtagPlaceholder}
            ...
            <div className="mb-4">
              {isInstagramConnected ? (
                <div className="flex items-center justify-center gap-2 text-sm text-green-700 bg-green-50 rounded-lg p-3 border border-green-200">
                  ...
                  <span>{t.library.icmConnectedTo} <strong>@{instagramUsername}</strong></span>
                </div>
              ) : (
                <div className="text-center">
                  <p className="text-sm text-amber-700 bg-amber-50 rounded-lg p-3 border border-amber-200 mb-3">
                    ⚠️ {t.library.icmConnectWarning}
                  </p>
                  <button
                  ...
                  >
                    ...
                    <span>{t.library.icmConnectInstagram}</span>
                  </button>
                </div>
              )}
            </div>
              ...
              <button
                ...
                className={`flex-1 px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-medium text-white transition-all flex items-center justify-center gap-2 text-sm sm:text-base ${
                  publishing || !caption.trim() || selectedImages.length < 2
                    ...
                    : 'bg-gradient-to-r from-[#0c1a3a] to-[#1e3a5f] hover:from-[#1e3a5f] hover:to-[#2a4a6f] shadow-lg hover:shadow-xl'
                }`}
              >
                {publishing ? (
                  <>
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                    <span>{t.library.icmPublishing}</span>
                  </>
                ) : (
                  <>
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <span>{t.library.icmPublishCarousel} ({selectedImages.length})</span>
                  </>
                )}
              </button>
            )}
