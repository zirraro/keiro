...
-- Date: 2026-02-02
-- Description: Allow instagram_drafts to store both images and videos (Reels), with category organization
...
BEGIN
  IF EXISTS (
    ...
    COMMENT ON COLUMN instagram_drafts.video_id IS 'Reference to my_videos if media_type is video (Reel)';
  END IF;
  ...
  IF EXISTS (
    ...
    COMMENT ON COLUMN instagram_drafts.media_type IS 'Type of media: image (post/carousel) or video (Reel)';
  END IF;
  ...
  IF EXISTS (
    ...
    COMMENT ON COLUMN instagram_drafts.category IS 'Category: draft (default), published (published to Instagram)';
  END IF;
  ...
  IF EXISTS (
    ...
    COMMENT ON COLUMN instagram_drafts.media_url IS 'URL of media (image or video)';
  END IF;
END $$;
